...
if (tab === "users") {
  ...
  // Kategorileri her açılışta VDS'den tazele; izin ekranı yalnızca o anki
  // ürün state'ine bağlı kalmasın.
  ...
  categoryLoader
    .then(() => renderUserCategoryPermissions())
    .catch(err => {
      console.warn("Yetki kategori listesi yenilenemedi:", err?.message || err);
      renderUserCategoryPermissions();
      showToast(err?.message || "Kategori listesi yenilenemedi", true);
    });
}
...
async function checkAppVersion({ silent = true } = {}) {
  ...
  try {
    ...
    console.info("[GarageFlow updater]", { runtimeVersion, remoteVersion, source: "navigation-v16.7.js" });
